...
public class DppApp {
	...
	public static void main(String[] args) {
		...
		do {
			...
			if(chooseManu!=0) {
				...
				if(action!=0 && action!=1) doAction(chooseManu, action);
			}
		...
		if(chooseManu==1) {
			...
			if(action==6) {
				...
				try {
					...
					if(employees.isEmpty()) System.out.println("no names or surname found with '"+letters+ "' letters");
			...
			if(action==7 ||action==8) {
				...
				try {
					...
					if(employees.isEmpty()) System.out.println("No one");
					else employees.forEach(System.out::println);
				} catch (SQLException e) {
					System.out.println("Problems with database :( ...");
					e.printStackTrace();
		...
		if(chooseManu==2) {
			...
			if(action==4 || action==5) {
				...
				try {
					...
					if(projects.isEmpty()) System.out.println("No one");
	...
	private static String inputLetters() {
		...
		do {
			...
			input = scanner.next();
			if(!ExtarFunctional.checkingTextOnlyLetters(input)) System.out.println("Please enter correctly, try again:");
			else result=true;
		} while (!result);
		return input;
	}
